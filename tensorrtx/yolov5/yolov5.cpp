...
int main(int argc, char** argv) {
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        auto read_image_time = 0;
        float* buffer_idx = (float*)buffers[inputIndex];
        for (int b = 0; b < fcount; b++) {
            auto start = std::chrono::system_clock::now();
            cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]);
            auto end = std::chrono::system_clock::now();
            read_image_time= std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        ...
        auto end = std::chrono::system_clock::now();
        std::cout << "inference time: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() -read_image_time << "ms" << std::endl;
        for (int b = 0; b < fcount; b++) {
            auto& res = batch_res[b];
